...
### tool_kit.sh - 下载与完整性校验稳健性修复

**问题现象**：在网络不稳定或证书校验失败时，`wget`可能中途失败，但仍留下部分下载的压缩包。后续续跑时阶段脚本仅按“文件是否存在”跳过下载，直接解压，导致 `tar` 报 `EOF`（截断文件）。

**根因定位**：
- 阶段脚本的通用流程为：`if [ -f ${pkg} ]; then echo "found"; else download; fi; tar ...`。当包已存在时不会再进行校验，直接解压。
- `download_pkg_from_url()`（以及备用的 `download_pkg_from_org()`）在 `wget` 失败时未清理输出文件，容易残留截断的压缩包；只有“下载成功”才进入 `checksum` 校验并在校验失败时删除文件。

**最小改动修复**（已实施）：
- 在 `scripts/tool_kit.sh` 的下载函数中，于所有 `wget` 失败分支添加 `rm -f "$__filename"`，确保失败后不留坏包。
  - 覆盖三种证书策略分支：`strict`、`skip`、`smart`（安全失败→不安全重试→不安全失败）。
  - 备用镜像函数 `download_pkg_from_org()` 的失败分支亦添加相同清理。
- 保持原有设计不变：仅在“下载成功”时执行 `checksum "$__filename" "$__sha256"`；校验失败仍删除坏包并返回错误。

**改动位置**：
- `scripts/tool_kit.sh`
  - `download_pkg_from_url()`：在各 `wget` 失败分支增加 `rm -f "$__filename"`（约行 957、968、984）。
  - `download_pkg_from_org()`：在 `wget` 失败分支增加 `rm -f "$__filename"`（约行 932）。

**设计思考**：
- 选择在工具层集中修复，避免触碰各 `install_*.sh` 阶段脚本的分散逻辑，改动面最小，风险最低。
- 与既有“校验失败删除文件”的语义一致，形成统一的坏包清理策略：下载失败清理、校验失败清理，均保证续跑不会错误跳过下载。
- 不改变“离线/混合安装”的既有探测逻辑：手工放置的正确包仍按原流程使用；错误包会在解压或后续校验处暴露并被清理。

**验证建议**：
- 人为制造下载失败（断网或错误 URL），观察失败后目标文件是否被删除；续跑应重新下载并成功解压。
- 回归样例：`fftw`（`tar -xzf`）、`openmpi`（`tar -xjf`）。删除历史坏包后重跑，确认不再出现 `EOF`。

**可进一步加固（保持最小改动原则下暂不实施）**：
- 在“已存在包→解压”路径前补充一次校验：存在时也调用 `checksum`，避免使用来源不明或历史残留的坏包。
- 解压前进行归档自检：`tar -tzf` / `tar -tjf` 预检查失败则删除并重新下载，减少解压阶段的失败噪音。
- 原子下载：下载至临时文件（如 `filename.part`），成功后 `mv` 成正式文件；失败则仅留下 `.part` 并清理，进一步降低残留概率。
- 更稳健的 downloader 默认：合理设置 `--tries`、`--timeout`；慎用 `--continue`（断点续传可能与远端内容不一致导致隐藏问题）。
- 信号清理：在下载阶段设置 `trap`，对 `INT`/`TERM` 中断进行临时文件清理，杜绝异常退出残留。

**影响评估**：
- 续跑稳定性显著提升：不会再因历史截断包导致 `EOF`。
- 变更范围小、行为可预期：仅清理失败下载的目标文件，其他逻辑与用户工作流不变。
